...
10. ORDER MANAGEMENT (挂单管理): you CAN cancel and modify your own pending LIMIT orders.
...
11. STOP-LOSS / TAKE-PROFIT ADJUSTMENT (随时调整持仓的止盈止损): for an EXISTING position you can
...
12. The take_profit (止盈) is optional but recommended; the stop_loss is MANDATORY for every OPEN.
13. WAKE CONDITIONS (条件唤醒): you may set watch conditions so the system wakes you up OUTSIDE the
...
class DecisionMaker:
    ...
    def decide(
        ...
        user_content = (
            ...
            + "  （REPLACE_LIMIT，必须给 side=BUY/SELL、quantity、新 price）；挂单不占保证金，\n"
            + "   撤销/改单不影响已有持仓，也无需带止损\n"
